...
// Ajoute une consultation. La saisie du diagnostic/notes reste reservee au medecin.
router.post('/', (req, res) => {
  ...
  // Une secretaire ne peut pas inscrire de diagnostic medical
  const diagnostic = req.user.role === 'medecin' ? d.diagnostic ?? null : null;
  const notes = req.user.role === 'medecin' ? d.notes ?? null : null;

  const info = db.prepare(`
    INSERT INTO consultations (patient_id, doctor_id, date, motif, diagnostic, notes)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(d.patient_id, doctorId, d.date, d.motif ?? null, diagnostic, notes);
